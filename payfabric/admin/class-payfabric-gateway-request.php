...
class PayFabric_Gateway_Request
{
     ...
     *
     */
    private function bizuno_get_wallet_link_id($cID=0)
    {
        if (!empty($cID)) {
            $wallet_id = get_user_meta($cID, 'bizuno_wallet_id', true);
            if (!empty($wallet_id)) { $cID = $wallet_id; }
        }
        return $cID;
    }
    ...
    private function get_payfabric_gateway_post_args($order)
    {
        ...
        if(!$customerId = $order->get_customer_id()){
            ...
            $customerId = $cookie[0];
        } else { // ********************* Added by PhreeSoft to link to Bizuno Wallet **************************
            $customerId = $this->bizuno_get_wallet_link_id($customerId);
        }

        return array(
            "id" => !empty($GLOBALS['payfabric_pending_order_id']) ? $GLOBALS['payfabric_pending_order_id'] : $order->get_id(), // REQUIRED - Merchant internal order id //
