...
class payFabric_ServiceBase
{
    ...
    public function setCredentials($mid = null, $key = null)
    {
        try {
            ...
            if (is_object(payFabric_RequestBase::$logger)) {
                payFabric_RequestBase::$logger->logFatal($e->getMessage() . " in " . $e->getFile() . " on line " . $e->getLine());
            }
            throw $e;
        }
    }
    ...
    public function setEnvironment($param = null)
    {
        try {
            if (strtoupper($param) == 'TEST' || $param) {
                ...
                throw new BadMethodCallException('[PayFabric Class error] Invalid environment. ' . __METHOD__ . ' accepts either "TEST" or "LIVE"', 400);
            }
            if (is_object(payFabric_RequestBase::$logger)) {
    ...
    {
        if (!isset($path)) {
            throw new Exception( esc_html ( sprintf( 'Logger path %s is required', $path) ) );
        }
